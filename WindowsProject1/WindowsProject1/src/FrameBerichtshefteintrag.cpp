...
std::vector <std::string> geloeschteTaetigkeitenBetrieb = {};
std::vector <std::string> geloeschteTaetigkeitenBetriebIDs = {};
std::vector <std::string> geloeschteTaetigkeitenSchule = {};
std::vector <std::string> geloeschteTaetigkeitenSchuleIDs = {};
...
wxDateTime datumdatetimevon = {};
wxDateTime datumdatetimebis = {};
...
void FrameBerichtshefteintrag::OnErsteTaetigkeitEntfernen(wxCommandEvent& event)
{
	const WrapperPanelTaetigkeit* wrapper1 = static_cast<WrapperPanelTaetigkeit*>(event.GetEventUserData());
	PanelTaetigkeitbase* panel1 = wrapper1->Get();
	const wxWindow* panelTaetigkeit1 = panel1->GetParent();
	...
	
{
	...
	panelbetriebneu->btn_delete_taetigkeit->Connect(wxEVT_COMMAND_BUTTON_CLICKED, wxCommandEventHandler(FrameBerichtshefteintrag::OnErsteTaetigkeitEntfernen), new
		WrapperPanelTaetigkeit(panelbetriebneu), this);
...
void FrameBerichtshefteintrag::panelschultaetigkeiterstellen()
{
	...
	panelschuleneu->btn_delete_taetigkeit->Connect(wxEVT_COMMAND_BUTTON_CLICKED, wxCommandEventHandler(FrameBerichtshefteintrag::OnErsteTaetigkeitEntfernen), new
		WrapperPanelTaetigkeit(panelschuleneu), this);
	...
	, _db(db)
{
	...
	datumdatetimevon = _calendarVon->GetDate();
	datumdatetimebis = _calendarBis->GetDate();
...
void FrameBerichtshefteintrag::ResetNameChoice () 
{
   ...
   for (const auto& i : azubi_tabelle.List()) {//für alle azubi_ids in azubi_tabelle werden
	   ...
	   auto test = _choiceName->GetClientData();
...
SELECT berichtsheft_id FROM berichtsheft WHERE woche_fk = ?
	...
	std::vector<float> auswahlberichtsheftminutenvector;
	...
	for (int i = 0; i < berichtsheftIDAnzahl; i++)
	{
		...
		//////
		floatminuten = berichtsheftwerte.minuten;
		auswahlberichtsheftminutenvector.push_back(floatminuten/60);
		//////
		auswahlberichtshefttaetigkeitfkvector.push_back(berichtsheftwerte.taetigkeit_fk);//berichtsheftwerte.id statt taetigkeit_fk
	...
	std::vector<std::string> idstaetigkeitenbetrieb;
	std::vector<std::string> idstaetigkeitenschule;

	std::vector<float> taetigkeitminutenvectorbetrieb;
	std::vector<float> taetigkeitminutenvectorschule;

	auto max_ID = max_element(std::begin(retBerichtsheftID), std::end(retBerichtsheftID));

	//Anstelle der nächsten beiden If-Blöcke die Betrieb und Schuleinträge schon vorher aufspalten

	if (anzahl_betriebeinträge > 0)
	{

		for (size_t i = 0; i < retBerichtsheftID.size(); ++i)
		{
			auto resbetriebstätigkeit = mk::sqlite::result{ _db, R"(
SELECT beschreibung FROM taetigkeit WHERE taetigkeit_id = ? AND art_fk = ?
)", retBerichtsheftID[i], 1 };
			while (resbetriebstätigkeit)
			{
				int ii = 0;
				taetigkeitvectorbetrieb.push_back(resbetriebstätigkeit[0]);
				++resbetriebstätigkeit;
			}
		}


		for (size_t i = 0; i < taetigkeitvectorbetrieb.size(); ++i)
		{
			auto residstaetigkeitenbetrieb = mk::sqlite::result{ _db, R"(
SELECT taetigkeit_id FROM taetigkeit WHERE beschreibung = ?
)", taetigkeitvectorbetrieb[i] };
			while (residstaetigkeitenbetrieb)
			{
				int ii = 0;
				idstaetigkeitenbetrieb.push_back(residstaetigkeitenbetrieb[0]);
				++residstaetigkeitenbetrieb;
			}
		}
		...
		for (size_t i = 0; i < taetigkeitvectorbetrieb.size(); ++i)
		{
			auto resminutenbetrieb = mk::sqlite::result{ _db, R"(
SELECT minuten FROM berichtsheft WHERE berichtsheft_id = ?
)", idstaetigkeitenbetrieb[i] };
			while (resminutenbetrieb)
			{
				
				int ii = 0;
				taetigkeitminutenvectorbetrieb.push_back(std::stof(resminutenbetrieb[0]));
				++resminutenbetrieb;
			}
		}
	...
	if (anzahl_schuleinträge > 0)
	{
		...
		for (size_t i = 0; i < retBerichtsheftID.size(); ++i)
		{
			auto resschultätigkeit = mk::sqlite::result{ _db, R"(
SELECT beschreibung FROM taetigkeit WHERE taetigkeit_id = ? AND art_fk = ?
)", retBerichtsheftID[i], 2 };
			while (resschultätigkeit)
			{
				int ii = 0;
				taetigkeitvectorschule.push_back(resschultätigkeit[0]);
				++resschultätigkeit;
			}
		}
		...
		for (size_t i = 0; i < taetigkeitvectorschule.size(); ++i)
		{
			auto residstaetigkeitenschule = mk::sqlite::result{ _db, R"(
SELECT taetigkeit_id FROM taetigkeit WHERE beschreibung = ?
)", taetigkeitvectorschule[i] };
			while (residstaetigkeitenschule)
			{
				int ii = 0;
				idstaetigkeitenschule.push_back(residstaetigkeitenschule[0]);
				++residstaetigkeitenschule;
			}
		}
		...
		for (size_t i = 0; i < taetigkeitvectorschule.size(); ++i)
		{
			auto resminutenschule = mk::sqlite::result{ _db, R"(
SELECT minuten FROM berichtsheft WHERE berichtsheft_id = ?
)", idstaetigkeitenschule[i] };
			while (resminutenschule)
			{

				int ii = 0;
				taetigkeitminutenvectorschule.push_back(std::stof(resminutenschule[0]));
				++resminutenschule;
			}
		}
	...
	if (taetigkeitvectorbetrieb.size() > 0)
	{
		for (auto& valuesweitergabebetrieb : _panelBetrieb->GetChildren())
		{
			...
			resultminutenbetrieb << taetigkeitminutenvectorbetrieb[0]/60;
	...
	if (taetigkeitvectorschule.size() > 0)
	{
		for (auto& valuesweitergabeschule : _panelSchule->GetChildren())
		{
			...
			resultminutenschule << taetigkeitminutenvectorschule[0]/60;
	...
	if (anzahl_betriebeinträge > 1)
	{
		for (int i = 1; i < anzahl_betriebeinträge; ++i)
		{
			...
			panelbetriebneu->btn_delete_taetigkeit->Connect(wxEVT_COMMAND_BUTTON_CLICKED, wxCommandEventHandler(FrameBerichtshefteintrag::OnTaetigkeitEntfernen), new
				WrapperPanelTaetigkeit(panelbetriebneu), this);
			...
			std::stringstream resultminutenbetrieb;
			resultminutenbetrieb << taetigkeitminutenvectorbetrieb[i]/60;
	...
	if (anzahl_schuleinträge > 1)
	{
		for (int i = 1; i < anzahl_schuleinträge; ++i)
		{
			...
			panelschuleneu->btn_delete_taetigkeit->Connect(wxEVT_COMMAND_BUTTON_CLICKED, wxCommandEventHandler(FrameBerichtshefteintrag::OnTaetigkeitEntfernen), new
				WrapperPanelTaetigkeit(panelschuleneu), this);
			...
			std::stringstream resultminutenschule;
			resultminutenschule << taetigkeitminutenvectorschule[i]/60;
...
void FrameBerichtshefteintrag::OnButtonNeuAbteilung(wxCommandEvent & /*event*/)
{
	...
	if (dlg.ShowModal() == wxID_OK) {
		...
		abteilung.name = dlg._eingabe_abteilung->GetValue().Trim().Trim(false);
...
void FrameBerichtshefteintrag::OnButtonNeuName(wxCommandEvent & /*event*/) 
{
   ...
   if (dlg.ShowModal() == wxID_OK) {
      ...
      auto azubi = Azubi{};
      azubi.vorname = dlg._eingabe_vorname->GetValue().Trim().Trim(false);
      azubi.nachname = dlg._eingabe_nachname->GetValue().Trim().Trim(false);
...

{
	...
	auto kalender_monat_von = kalendar_auswahl.GetMonth();
	...
	if (wxAtoi(tag) == 1)
	{
		...
		//hier abfragen ob der Monat verändert wurde

		auto kalender_monat_vorher = datumdatetimevon.GetMonth();

		if (kalender_monat_von == kalender_monat_vorher)
		{
...
void FrameBerichtshefteintrag::OnCalendarBisChanged(wxCalendarEvent & /*event*/)
{
	...
	auto kalender_monat_bis = kalendar_auswahl.GetMonth();
	...
	if (wxAtoi(tag) == 5)
	{
		...
		auto kalender_monat_vorher = datumdatetimebis.GetMonth();

		if (kalender_monat_bis == kalender_monat_vorher)
		{
...
void FrameBerichtshefteintrag::OnBetriebTaetigkeitErstellen(wxCommandEvent& event) {
	...
	_paneltaetigkeitbetriebneu->btn_delete_taetigkeit->Connect(wxEVT_COMMAND_BUTTON_CLICKED, wxCommandEventHandler(FrameBerichtshefteintrag::OnTaetigkeitEntfernen), new
...
void PanelTaetigkeitbase::OnTextComboStundenChanged(wxCommandEvent & event)
{
	...
	/*
	if (regexstring.find('.' | ',') != std::string::npos)
	{
		wxLogMessage("Keine Dezimalzahlen eingeben");
		combo_stunden->SetBackgroundColour(*wxRED);
	}
...
void FrameBerichtshefteintrag::OnSchuleTaetigkeitErstellen(wxCommandEvent & event)
{
	...
	_paneltaetigkeitschuleneu->btn_delete_taetigkeit->Connect(wxEVT_COMMAND_BUTTON_CLICKED, wxCommandEventHandler(FrameBerichtshefteintrag::OnTaetigkeitEntfernen), new
...
DELETE FROM taetigkeit WHERE taetigkeit_id = ?
	...
	datetime.SetToWeekDayInSameWeek(wxDateTime::Fri);
...
void FrameBerichtshefteintrag::OnButtonUpdaten(wxCommandEvent & event)//Auswerten der combo_stunden Panele suchen und abfragen ob ausgefüllt und ggf die weitere Ausführung des Programmes stoppen
{
...
SELECT taetigkeit_fk FROM berichtsheft WHERE woche_fk = ?
	...
	//Die Tätigkeiten auslesen die zu Betrieb und die zu Schule gehören
	std::vector<std::string>retBetriebtaetigkeitenInDatenbank = {};
	std::vector<std::string>retSchultaetigkeitenInDatenbank = {};


	//Die betrieblichen Tätigkeiten die schon in der Datenbank stehen
	for (size_t i = 0; i < rettaetigkeitenIDs.size(); i++)
	{
		auto restaetigkeitbetriebvec = mk::sqlite::result{ _db, R"(
SELECT beschreibung FROM taetigkeit WHERE taetigkeit_id = ? AND art_fk = ?
)", rettaetigkeitenIDs[i], 1 };
		while (restaetigkeitbetriebvec)
		{
			int ii = 0;
			retBetriebtaetigkeitenInDatenbank.push_back(restaetigkeitbetriebvec[0]);
			++restaetigkeitbetriebvec;
		}
	}

	//Die schulischen Tätigkeiten die schon in der Datenbank stehen
	for (size_t u = 0; u < rettaetigkeitenIDs.size(); u++)
	{
		auto restaetigkeitschulvec = mk::sqlite::result{ _db, R"(
SELECT beschreibung FROM taetigkeit WHERE taetigkeit_id = ? AND art_fk = ?
)", rettaetigkeitenIDs[u], 2 };
		while (restaetigkeitschulvec)
		{
			int ii = 0;
			retSchultaetigkeitenInDatenbank.push_back(restaetigkeitschulvec[0]);
			++restaetigkeitschulvec;
		}
	}

	//Die Anzahl aller bereits vorhandenen Tätigkeit_IDs heruasfinden

	int64_t hoechsteTaetigkeitID = 0;

	std::vector<int64_t> alleTaetigkeitIDsVec = {};	
	auto resAlleTaetigkeitIDsVec = mk::sqlite::result{ _db, R"(
SELECT taetigkeit_id FROM taetigkeit
)" };
	while (resAlleTaetigkeitIDsVec)
	{
		int i = 0;
		alleTaetigkeitIDsVec.push_back(resAlleTaetigkeitIDsVec[i]);
		++resAlleTaetigkeitIDsVec;
		if (alleTaetigkeitIDsVec.back() > hoechsteTaetigkeitID)
		{
			hoechsteTaetigkeitID = alleTaetigkeitIDsVec.back();
		}
	}
	...
	std::vector<int> Betriebstundenint = {};


	float minuten = {};
	std::string minutenstring = {};

	auto count = 1;
	bool isstundevorhanden = true;
	...
	for (auto& valuesbetrieb : _panelBetrieb->GetChildren())
	{
		...
		///////////////
		///////////////
		///////////////hier weitermachen und das Programm so beenden dass man wieder etwas eingeben kann und 
		/////////////// der Dialog mit der Info aufploppt
		///////////////
		///////////////

		minuten = std::atof(panel->combo_stunden->GetValue())*60;

		if (minuten == 0)
		{
			wxcount << count;
			isstundevorhanden = false;
			break;
		}


		minutenstring = std::to_string(minuten);


		//hier Minutenstring mit regex überprüfen


		Betriebstunden.push_back(minutenstring);

		
		if (Betriebstätigkeitenvector.size() > Betriebstunden.size())
		{
			wxLogError("Bitte bei den Schultätigkeiten überall die Stunden mit angeben.");
			return;
		}
	...
	//Abfrage mit bool
	if (isstundevorhanden == false)
	{
		wxLogMessage(wxT("Die Eingabe der Minuten für die Betriebstätigkeit in Zeile: \n") + wxcount + wxT("\nist ungültig. Bitte eine ganze Zahl eingeben."));
		wxcount.clear();
		return;
	}
	...
	for (auto& valuesschule : _panelSchule->GetChildren())
	{
		...
		minuten = std::atof(panel->combo_stunden->GetValue()) * 60;
		minutenstring = std::to_string(minuten);
		Schulstunden.push_back(minutenstring);
	}
	...
	//Hier die korrekten Betrieb und SchultätigkeitsIDs auslesen
	for (size_t i = 0; i < retBetriebtaetigkeitenInDatenbank.size(); ++i)
	{
		auto residstaetigkeitenbetriebindatenbank = mk::sqlite::result{ _db, R"(
SELECT taetigkeit_id FROM taetigkeit WHERE beschreibung = ? 
)", retBetriebtaetigkeitenInDatenbank[i] };
		while (residstaetigkeitenbetriebindatenbank)
		{
			int ii = 0;
			TaetigkeitBetriebIDs.push_back(residstaetigkeitenbetriebindatenbank[0]);
			++residstaetigkeitenbetriebindatenbank;
		}
	}
	
	for (size_t i = 0; i < retSchultaetigkeitenInDatenbank.size(); ++i)
	{
		auto residstaetigkeitenschuleindatenbank = mk::sqlite::result{ _db, R"(
SELECT taetigkeit_id FROM taetigkeit WHERE beschreibung = ?
)", retSchultaetigkeitenInDatenbank[i] };
		while (residstaetigkeitenschuleindatenbank)
		{
			int ii = 0;
			TaetigkeitSchuleIDs.push_back(residstaetigkeitenschuleindatenbank[0]);
			++residstaetigkeitenschuleindatenbank;
		}
	}
...
SELECT woche_id FROM woche WHERE beginn = ? AND ende = ?
		...
		geloeschteTaetigkeitenBetriebIDs.clear();
		
		auto restaetifkeitfk = mk::sqlite::result{ _db, R"(
SELECT taetigkeit_fk FROM berichtsheft WHERE woche_fk = ?
)", retDatum[0] };
		while (restaetifkeitfk)
		{
			int i = 0;
			geloeschteTaetigkeitenBetriebIDs.push_back(restaetifkeitfk[i]);
			++restaetifkeitfk;
		}

		_panelBetrieb->Refresh();


		//auto test = _panelBetrieb->GetChildren();

		geloeschteTaetigkeitenBetrieb.clear();
		...
		for (const auto& valuesbetrieb : _panelBetrieb->GetChildren()) {
			...
			geloeschteTaetigkeitenBetrieb.push_back(beschreibung.ToStdString());
		}

		//Die IDs der noch vorhandenen Betriebstätigkeiten
		std::vector<std::string> rettatsaechlichvorhandeneTaetigkeitenBetrieb = {};

		for (size_t i = 0; i < geloeschteTaetigkeitenBetrieb.size(); i++)
		{
			auto restatsaechlicheIDsBetrieb = mk::sqlite::result{ _db, R"(
	SELECT taetigkeit_id FROM taetigkeit WHERE beschreibung = ? AND art_fk = 1
	)", geloeschteTaetigkeitenBetrieb[i] };


		while (restatsaechlicheIDsBetrieb)
			{
				int i = 0;
				rettatsaechlichvorhandeneTaetigkeitenBetrieb.push_back(restatsaechlicheIDsBetrieb[i]);
				++restatsaechlicheIDsBetrieb;
			}
		}

		//mit TaetigkeitBetriebIDs abgleichen
		std::vector<int64_t> zuloeschendeIDs = {};

		for (auto& value : TaetigkeitBetriebIDs)
		{
			if (!(std::find(rettatsaechlichvorhandeneTaetigkeitenBetrieb.begin(), rettatsaechlichvorhandeneTaetigkeitenBetrieb.end(), std::to_string(value)) != rettatsaechlichvorhandeneTaetigkeitenBetrieb.end()))
			{
				zuloeschendeIDs.push_back(value);
			}
		}

		for (int64_t i : zuloeschendeIDs)
		{
			TaetigkeitBetriebIDs.erase(std::remove(TaetigkeitBetriebIDs.begin(), TaetigkeitBetriebIDs.end(), i), TaetigkeitBetriebIDs.end());
		}

	
		//alle Taetigkeiten mit ID im Vektor löschen

		for (size_t i = 0; i < zuloeschendeIDs.size(); i++)
		{
			mk::sqlite::execute(_db, R"(
DELETE FROM taetigkeit WHERE taetigkeit_id = ?
)", zuloeschendeIDs[i]);
			...
			mk::sqlite::execute(_db, R"(
DELETE FROM berichtsheft WHERE taetigkeit_fk = ?
)", zuloeschendeIDs[i]);
		}

		wxLogMessage("Es wurde mindestens ein Betriebspanel entfernt");
	}
...
SELECT woche_id FROM woche WHERE beginn = ? AND ende = ?
		...
		geloeschteTaetigkeitenSchuleIDs.clear();
...
SELECT taetigkeit_fk FROM berichtsheft WHERE woche_fk = ?
		...
		{
			int i = 0;
			geloeschteTaetigkeitenSchuleIDs.push_back(restaetifkeitfk[i]);
			++restaetifkeitfk;
		}

		_panelSchule->Refresh();

		geloeschteTaetigkeitenSchule.clear();
		...
		for (const auto& valuesschule : _panelSchule->GetChildren()) {
			...
			geloeschteTaetigkeitenSchule.push_back(beschreibung.ToStdString());
		}

		

		//Die IDs der noch vorhandenen Betriebstätigkeiten
		std::vector<std::string> rettatsaechlichvorhandeneTaetigkeitenSchule = {};

		for (size_t i = 0; i < geloeschteTaetigkeitenSchule.size(); i++)
		{
			auto restatsaechlicheIDsSchule = mk::sqlite::result{ _db, R"(
	SELECT taetigkeit_id FROM taetigkeit WHERE beschreibung = ? AND art_fk = 2
	)", geloeschteTaetigkeitenSchule[i] };

		
			

			while (restatsaechlicheIDsSchule)
			{
				int i = 0;
				rettatsaechlichvorhandeneTaetigkeitenSchule.push_back(restatsaechlicheIDsSchule[i]);
				++restatsaechlicheIDsSchule;
			}
		}
		...
		for (auto& value : TaetigkeitSchuleIDs)
		{
			if (!(std::find(rettatsaechlichvorhandeneTaetigkeitenSchule.begin(), rettatsaechlichvorhandeneTaetigkeitenSchule.end(), std::to_string(value)) != rettatsaechlichvorhandeneTaetigkeitenSchule.end()))
			{
				zuloeschendeIDs.push_back(value);
			}
		}
...
DELETE FROM berichtsheft WHERE taetigkeit_fk = ?
		...
		wxLogMessage("Es wurde mindestens ein Schulpanel entfernt");
	}

	if (Betriebstunden.size() > TaetigkeitBetriebIDs.size())
	{
		wxLogMessage("Bitte überall die Minuten mit angeben");
	}
	...
	if (TaetigkeitBetriebIDs.size() < Betriebstätigkeitenvector.size())
	{
		...
		//wxLogMessage("Es müssen noch weitere Panele mit Betrieblichen Tätigkeiten in die Datenbank übernommen werden");

		size_t iterator = 0;

		for (size_t i = TaetigkeitBetriebIDs.size(); i < Betriebstätigkeitenvector.size(); ++i)
		{
			//if (Betriebstunden.s)
...
)", Betriebstätigkeitenvector[TaetigkeitBetriebIDs.size() + iterator], 1);

			//Hier die Exception abfangen bei falschem Datentyp in den Stunden bei Updatefunktion
			mk::sqlite::execute(_db, R"(
INSERT INTO berichtsheft (minuten, woche_fk, taetigkeit_fk, azubi_fk) VALUES (?, ?, ?, ?)
)", Betriebstunden[TaetigkeitBetriebIDs.size() + iterator], wochenupdateid, hoechsteTaetigkeitID += 1, nameupdateid);
	...
	if (TaetigkeitSchuleIDs.size() < Schultätigkeitsvector.size())
	{
		//wxLogMessage("Es müssen noch weitere Panele mit Schulischen Tätigkeiten in die Datenbank übernommen werden");

		size_t iterator = 0;

		for (size_t i = TaetigkeitSchuleIDs.size(); i < Schultätigkeitsvector.size(); ++i)
		{
			mk::sqlite::execute(_db, R"(
INSERT INTO taetigkeit (beschreibung, art_fk) VALUES (?, ?)
)", Schultätigkeitsvector[TaetigkeitSchuleIDs.size() + iterator], 2);


			mk::sqlite::execute(_db, R"(
INSERT INTO berichtsheft (minuten, woche_fk, taetigkeit_fk, azubi_fk) VALUES (?, ?, ?, ?)
)", Schulstunden[TaetigkeitSchuleIDs.size() + iterator], wochenupdateid, hoechsteTaetigkeitID += 1, nameupdateid);
	...
	//Betriebstätigkeitenvector;
	//Schultätigkeitsvector;
	...
	if (anzahlchildrenbetrieb.GetCount() == TaetigkeitBetriebIDs.size())
	{
...

		{
			...
			*/
			...
			auto taetigkeits_tabelle = TaetigkeitTabelle{ _db };
			auto taetigkeitswerte = taetigkeits_tabelle.Load(idsBetrieb);
			taetigkeitswerte.beschreibung = Betriebstätigkeitenvector[i];
			taetigkeits_tabelle.Save(taetigkeitswerte);

			auto berichtsheft_tabelle = BerichtsheftTabelle{ _db };
			auto berichtsheftwerte = berichtsheft_tabelle.Load(idsBetrieb);
			berichtsheftwerte.minuten = std::stoll(Betriebstunden[i]);
			//berichtsheftwerte.taetigkeit_fk = TaetigkeitBetriebIDs[i];
			berichtsheft_tabelle.Save(berichtsheftwerte);
	...
	if (anzahlchildrenschule.GetCount() == TaetigkeitSchuleIDs.size())
	{
		for (const int64_t& idsSchule : TaetigkeitSchuleIDs)
		{
			...
			*/
			...
			auto taetigkeits_tabelle = TaetigkeitTabelle{ _db };
			auto taetigkeitswerte = taetigkeits_tabelle.Load(idsSchule);
			taetigkeitswerte.beschreibung = Schultätigkeitsvector[i];
			taetigkeits_tabelle.Save(taetigkeitswerte);

			auto berichtsheft_tabelle = BerichtsheftTabelle{ _db };
			auto berichtsheftwerte = berichtsheft_tabelle.Load(idsSchule);
			berichtsheftwerte.minuten = std::stoll(Schulstunden[i]);
			//berichtsheftwerte.taetigkeit_fk = TaetigkeitSchuleIDs[i];
			berichtsheft_tabelle.Save(berichtsheftwerte);
...
SELECT beschreibung FROM taetigkeit WHERE taetigkeit_id = ? AND art_fk = ?
...

	   {
		   ...
		   std::vector<int> Betrieb_minuten_vec_int;

		   for (size_t i = 0; i < Betrieb_minuten_vec.size(); i++)
		   {
			   Betrieb_minuten_vec_int.push_back(std::stoi(Betrieb_minuten_vec[i])/60);
		   }
		   ...
		   for (size_t i = 0; i < anzahl_betriebstaetigkeiten; ++i)
		   {
			   ...
			   //Hier den Betrieb_minuten_vec in int und dann durch 60 teilen
			   panel->combo_stunden->SetLabel(std::to_string(Betrieb_minuten_vec_int[i]));
		   }
		   ...
		   std::vector<float> Schule_minuten_vec_float;

		   for (size_t i = 0; i < Schule_minuten_vec.size(); i++)
		   {
			   Schule_minuten_vec_float.push_back(std::stof(Schule_minuten_vec[i]) / 60);
		   }
		   ...
		   for (size_t i = 0; i < anzahl_schulischetaetigkeiten; ++i)
		   {
			   ...
			   //Float kürzen
			   //finde den Punkt
			   int lengthfloat = Schule_minuten_vec[i].length();
			   auto teststring = (std::to_string(Schule_minuten_vec_float[i])).substr(0, lengthfloat + 1);

			   
			   panel->combo_stunden->SetLabel(teststring);
		   }
...
SELECT name FROM art 
	...
	for (const auto& valuesschule : _panelSchule->GetChildren()) {
		...
		if (trigger == false)
		{
			wxLogMessage("Alle Tätigkeitsfelder müssen ausgefüllt sein");
		}

		else
		{
			wxLogMessage("Neuer Bericht angelegt");
			trans.commit();
		}
