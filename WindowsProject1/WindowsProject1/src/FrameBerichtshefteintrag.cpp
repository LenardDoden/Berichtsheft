...
public:
	...
	void drawcontenttable(std::string bezeichnung, std::vector<BeschreibungStunde> daten)
	{
		...
		for (auto zeile : daten)
		{
			...
			int splitmaﬂwidth = 520;
			int splitmaﬂchar = 80;



			if (textlaenge.GetWidth() > splitmaﬂwidth)
			{

				auto anzahlzeilen = textlaenge.GetWidth() / splitmaﬂwidth;
				...
				{
					splitstringvector.push_back(zeile.beschreibung.substr(i*splitmaﬂchar, splitmaﬂchar));
				}
				...
				for (auto x : splitstringvector)
				{
					...
					if (cursor.y > 950)
					{
						...
						dc->DrawText("Cursor.Y ist grˆﬂer als 2", leftMargin + 3, cursor.y + dc->GetCharHeight() / 2);
